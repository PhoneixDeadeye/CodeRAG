...
def test_process_repo_files(tmp_path):
    ...
    with patch("app.services.ingest.GenericLoader") as mock_loader:
        mock_instance = MagicMock()
        mock_instance.load.return_value = [
            Document(
                page_content="def hello(): pass",
                metadata={"source": str(repo_path / "test.py")},
            )
        ]
        mock_loader.from_filesystem.return_value = mock_instance

        texts = ingest._process_repo_files(
            str(repo_path), "http://github.com/test/repo"
        )
...
@patch("app.services.ingest.git.Repo")
def test_load_and_index_repo_clone(mock_repo, tmp_path):
    ...
    with patch("app.services.ingest._process_repo_files") as mock_process:
        mock_process.return_value = [Document(page_content="test")]

        texts = ingest.load_and_index_repo(
            url, repo_path, cache_path, force_reindex=True
        )
