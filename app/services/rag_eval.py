...
class EvaluationSuite:
    ...
    def get_summary(self) -> Dict:
        ...
        return {
            ...
            "average_metrics": {
                "context_relevance": sum(r.context_relevance for r in self.results)
                / len(self.results),
                "answer_relevance": sum(r.answer_relevance for r in self.results)
                / len(self.results),
                "faithfulness": sum(r.faithfulness for r in self.results)
                / len(self.results),
                "source_coverage": sum(r.source_coverage for r in self.results)
                / len(self.results),
            },
            "average_timing": {
                "retrieval_ms": sum(r.retrieval_time_ms for r in self.results)
                / len(self.results),
                "generation_ms": sum(r.generation_time_ms for r in self.results)
                / len(self.results),
                "total_ms": sum(r.total_time_ms for r in self.results)
                / len(self.results),
            },
        }
...
class RAGEvaluator:
    ...
    def _get_chain(self):
        ...
        if self._chain is None:
            ...
            self._chain = rag_engine.get_cached_qa_chain(self.collection_name)
        return self._chain
    ...
    async def evaluate_query(
        ...
        try:
            ...
            result.retrieval_time_ms = (
                (generation_end - retrieval_start) * 1000 * 0.3
            )  # Estimate
    ...
    def _calculate_context_relevance(self, query: str, sources: List[Dict]) -> float:
        """
    ...
    def _calculate_source_coverage(
        self, expected_sources: List[str], actual_sources: List[Dict]
    ) -> float:
