...
async def check_ai_service_health() -> dict:
    ...
    Returns:
        dict with status, provider, and optional error message
    """
    import asyncio
    ...
    try:
        # Try to get the client - this validates API key
        client = get_llm_client(provider)

        # Quick test with minimal token usage
        def sync_test():
            try:
                response = client.invoke("Say 'ok'")
                return True, response[:50] if response else "empty"
        ...
        success, response = await asyncio.wait_for(
            loop.run_in_executor(None, sync_test), timeout=10.0
        )
        ...
        result["error"] = "AI service did not respond within 10 seconds"
