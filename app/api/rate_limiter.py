    ...
    def cleanup_old_requests(self, window_seconds: int) -> None:
        ...
        for user_id in list(self.requests.keys()):
            ...
            self.requests[user_id] = [
                req for req in user_requests if req >= now - window_seconds
            ]
...
def check_chat_limit(tenant_id: str) -> None:
    """Check if tenant has exceeded chat rate limit."""
    if not chat_limiter.is_allowed(
        tenant_id, limit=CHAT_RATE_LIMIT, window_seconds=RATE_LIMIT_WINDOW
    ):
        remaining = chat_limiter.get_remaining(
            tenant_id, CHAT_RATE_LIMIT, RATE_LIMIT_WINDOW
        )
        raise HTTPException(
...
def check_ingest_limit(tenant_id: str) -> None:
    """Check if tenant has exceeded ingestion rate limit."""
    if not ingest_limiter.is_allowed(
        tenant_id, limit=INGEST_RATE_LIMIT, window_seconds=RATE_LIMIT_WINDOW
    ):
        remaining = ingest_limiter.get_remaining(
            tenant_id, INGEST_RATE_LIMIT, RATE_LIMIT_WINDOW
        )
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
...
LOGIN_RATE_LIMIT_WINDOW = 60  # 1 minute


def check_login_limit(ip_address: str) -> None:
    """Check if IP has exceeded login attempt rate limit."""
    if not login_limiter.is_allowed(
        ip_address, limit=LOGIN_RATE_LIMIT, window_seconds=LOGIN_RATE_LIMIT_WINDOW
    ):
        remaining = login_limiter.get_remaining(
            ip_address, LOGIN_RATE_LIMIT, LOGIN_RATE_LIMIT_WINDOW
        )
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            detail=f"Too many login attempts ({LOGIN_RATE_LIMIT} per minute). Please try again later.",
            headers={"X-RateLimit-Remaining": str(remaining)},
        )
