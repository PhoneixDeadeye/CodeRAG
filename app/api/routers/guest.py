...
from app.core.database import get_db, User, ChatSession, ChatMessage
...
async def merge_guest_session(
    ...
    new_session = ChatSession(
        ...
        name=f"Imported from Guest - {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M')}",
    )
    ...
    try:
        ...
        for msg in request.messages:
            ...
            chat_msg = ChatMessage(
                session_id=new_session.id, role=msg.role, content=msg.content.strip()
            )
            db.add(chat_msg)
            messages_imported += 1
        ...
        raise HTTPException(
            status_code=500, detail="Failed to merge session. Please try again."
        )
...
async def guest_status():
    ...
    return {
        ...
        "features": ["chat", "file_browse", "search", "repo_ingest"],
        "persistence": False,
