...
def generate_markdown(session: ChatSession, messages: list) -> str:
    ...
    lines.extend(
        [
            "",
            f"*Exported from CodeRAG on {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}*",
        ]
    )
...
"""
    ...
    html += f"""
    <div class="footer">
        Exported from CodeRAG on {datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")}
    </div>
</body>
</html>
"""
...
async def export_session_markdown(
    ...
    # Create filename
    safe_name = "".join(
        c for c in (session.name or "chat") if c.isalnum() or c in " -_"
    )[:50]
