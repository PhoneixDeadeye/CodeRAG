...
from app.core.utils import validate_safe_path, generate_github_link
...
async def get_repo_for_user_or_guest(
    repo_id: Optional[str], user: Optional[User], db: AsyncSession
) -> Optional[Repository]:
    """
...
async def get_file_tree(
    ...
    """
    repo = await get_repo_for_user_or_guest(repo_id, user, db)
    ...
    if not repo:
        ...
        if user is None:
            return {
                "tree": [],
                "repo_url": None,
                "message": "No repository available. Please ingest one first.",
            }
...
async def get_diff(
    ...
    repo = await get_repo_for_user_or_guest(request.repo_id, user, db)
    if not repo:
        raise HTTPException(status_code=404, detail="Repo not found")
    ...
    if os.path.exists(repo.local_path):
        for root, _, files in os.walk(repo.local_path):
            if ".git" in root or "node_modules" in root:
                continue
            for file in files:
                if file == os.path.basename(request.file_path):
                    continue
                ...
                try:
                    ...
                    logger.debug(
                        f"Failed to read file {fpath} for dependency analysis: {e}"
                    )
                    continue
