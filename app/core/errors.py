...
class APIError(Exception):
    ...
    def __init__(self, error: str, detail: str, code: str, status_code: int = 400):
        self.error = error
        self.detail = detail
        self.code = code
...
def not_found(resource: str, resource_id: str = "") -> APIError:
    ...
    return APIError(error="Not Found", detail=detail, code="NOT_FOUND", status_code=404)
...
def unauthorized(detail: str = "Authentication required") -> APIError:
    ...
    return APIError(
        error="Unauthorized", detail=detail, code="UNAUTHORIZED", status_code=401
    )
...
def rate_limited(detail: str = "Rate limit exceeded") -> APIError:
    ...
    return APIError(
        error="Too Many Requests", detail=detail, code="RATE_LIMITED", status_code=429
    )
...
async def generic_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    """Handle unhandled exceptions to prevent stack trace leakage."""
    request_id = getattr(request.state, "request_id", None)
