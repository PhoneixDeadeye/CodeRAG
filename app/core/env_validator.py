...
class EnvValidator:
    ...
    def warn_if_default(
        self, var_name: str, default_value: str, description: str = ""
    ) -> str:
    ...
    def validate_google_api_key(self) -> bool:
        ...
        api_key = self.require(
            "GOOGLE_API_KEY", "Required for cloud-based LLM (Gemini)."
        )
    ...
    def validate_all(self, strict: bool = False) -> bool:
        ...
        # Check for errors
        if self.errors:
            logger.error("[FAILED] Environment validation failed:")
        ...
        # Print warnings
        if self.warnings:
            logger.warning("[WARNING] Environment warnings:")
