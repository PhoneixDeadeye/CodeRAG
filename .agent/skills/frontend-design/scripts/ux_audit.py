...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # Familiar patterns
        if has_form and "<input" in content:
            # Quick check if it has any label/placeholder
            has_standard_labels = bool(
                re.search(r"<label|placeholder=|aria-label=", content, re.IGNORECASE)
            )
            # If it has inputs but no discernible labels/placeholders anywhere 
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [
                d
                for d in dirs
                if d not in {"node_modules", ".git", "dist", "build", ".next", "data", ".venv", "__pycache__", ".pytest_cache", "playwright-report", "test-results"}
            ]
            for file in files:
